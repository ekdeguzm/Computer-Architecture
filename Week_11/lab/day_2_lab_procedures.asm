...
mymul:  
	...
	subi	$sp, $sp, 32
	sw	$ra, 28($sp)
	sw	$s0, 24($sp)		# unlocks $s0 to be able to be used
	sw	$s1, 20($sp)		
	sw	$s2, 16($sp)
	
	# make sure that the multiplier and multiplicand are saved
	move	$s1, $a1
	move	$s2, $a0
	...
	lw	$ra, 28($sp)
	lw	$s0, 24($sp)		# returns $s0's value back to what it was
	lw	$s1, 20($sp)		
	lw	$s1, 16($sp)
	addi	$sp, $sp, 32
